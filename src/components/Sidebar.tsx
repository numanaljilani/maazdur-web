...
const Sidebar = ({ isOpen, toggleSidebar }: { isOpen: boolean; toggleSidebar: () => void }) => {
  ...
  const tabs = [
      { name: language ? 'घर' : 'Home', path: '/home', icon: '/home.png' },
      { name: language ? 'पसंदीदा' : 'Favorite', path: '/favorite', icon: '/bookmark.png' },
    { name: language ? 'उपयोगकर्ता' : 'User', path: '/profile', icon: '/user.png' },
    { name: language ? 'प्रोफ़ाइल संपादित करें' : 'Edit Profile', path: '/edit-profile', icon: '/edit.png' },
    { name: language ? 'ठेकेदार' : 'Contractor', path: '/contractor-registration', icon: '/suitcase.png' },
    { name: language ? 'गोपनीयता' : 'Privacy', path: '/privacy-policy', icon: '/secure-shield.png' },
    { name: language ? 'सहायता केंद्र' : 'Help Center', path: '/help-center', icon: '/available.png' },
    ...
    >
      <nav className="space-y-2">
        ...
        <div className='py-5'>
          ...
          <h4 className='text-xs text-gray-600'>{userData?.email}</h4>
        </div>
        </div>
        {tabs.map((tab) => (
          <Link
